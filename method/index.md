...
2. Initialise the weights of the network. These will be randomly generated numbers, our starting point. As the network runs these weights become optimised as the neural network learns which information is important to correctly guess the catagory of the object. The weights connect each node in the previous layer to each node in the present layer, thus the weights are a matrix with size ((N <span style="vertical-align: baseline; position: relative;top: -0.5em;>layer_i</span> +1) x N <sub>layer_j</sub> ) where j = i+1. NB there is an extra node added in layer_i which is the zero node or base unit.
